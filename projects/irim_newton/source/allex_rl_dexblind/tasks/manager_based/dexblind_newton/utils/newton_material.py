...
def set_shape_contact_stiffness(
    ...
    """Set contact stiffness (ke/kd) for all collision shapes of an asset.

    - 단위: ke [N/m], kd [N·s/m] → 접촉력 F = ke·d + kd·v 는 [N].
    - 내부에서 MuJoCo solref로 변환. ke/kd 둘 다 > 0 이어야 적용.
    - 실제 강성은 solimp.dmax·커널 해석에 의존. kinematic 상대편이면 침투 불가.
    """
    model = NewtonManager._model
    if model is None:
        return
