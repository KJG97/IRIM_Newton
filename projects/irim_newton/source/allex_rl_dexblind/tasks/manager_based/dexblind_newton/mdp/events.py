...
# ---------------------------------------------------------------------------
# Reset joints by offset (arm/hand separate ranges, sets PD targets)
# ---------------------------------------------------------------------------

def reset_joints_by_offset_arm_hand(
    env: "ManagerBasedEnv",
    env_ids: torch.Tensor,
    asset_cfg: SceneEntityCfg,
    arm_joint_names: list[str],
    hand_joint_names: list[str],
    position_range_arm: tuple[float, float],
    velocity_range_arm: tuple[float, float],
    position_range_hand: tuple[float, float],
    velocity_range_hand: tuple[float, float],
) -> None:
    """Reset arm and hand joints with separate random offset ranges and set PD targets.

    Uses default joint state, applies arm range to arm joints and hand range to hand
    joints, clamps to limits, then writes state and PD targets so the effect is not
    overwritten by _reset_articulations.
    """
    asset: Articulation = env.scene[asset_cfg.name]
    _, _, arm_ids = asset.find_joints(arm_joint_names, preserve_order=True)
    _, _, hand_ids = asset.find_joints(hand_joint_names, preserve_order=True)

    default_pos = wp.to_torch(asset.data.default_joint_pos)[env_ids].clone()
    default_vel = wp.to_torch(asset.data.default_joint_vel)[env_ids].clone()
    pos_limits = wp.to_torch(asset.data.soft_joint_pos_limits)[env_ids]
    vel_limits = wp.to_torch(asset.data.soft_joint_vel_limits)[env_ids]

    if arm_ids:
        arm_ids_t = torch.tensor(arm_ids, device=asset.device, dtype=torch.long)
        default_pos[:, arm_ids_t] += math_utils.sample_uniform(
            *position_range_arm, (len(env_ids), len(arm_ids)), default_pos.device
        )
        default_vel[:, arm_ids_t] += math_utils.sample_uniform(
            *velocity_range_arm, (len(env_ids), len(arm_ids)), default_vel.device
        )
        default_pos[:, arm_ids_t] = default_pos[:, arm_ids_t].clamp_(
            pos_limits[:, arm_ids_t, 0], pos_limits[:, arm_ids_t, 1]
        )
        default_vel[:, arm_ids_t] = default_vel[:, arm_ids_t].clamp_(
            -vel_limits[:, arm_ids_t], vel_limits[:, arm_ids_t]
        )
    if hand_ids:
        hand_ids_t = torch.tensor(hand_ids, device=asset.device, dtype=torch.long)
        default_pos[:, hand_ids_t] += math_utils.sample_uniform(
            *position_range_hand, (len(env_ids), len(hand_ids)), default_pos.device
        )
        default_vel[:, hand_ids_t] += math_utils.sample_uniform(
            *velocity_range_hand, (len(env_ids), len(hand_ids)), default_vel.device
        )
        default_pos[:, hand_ids_t] = default_pos[:, hand_ids_t].clamp_(
            pos_limits[:, hand_ids_t, 0], pos_limits[:, hand_ids_t, 1]
        )
        default_vel[:, hand_ids_t] = default_vel[:, hand_ids_t].clamp_(
            -vel_limits[:, hand_ids_t], vel_limits[:, hand_ids_t]
        )
    ...
    asset.set_joint_velocity_target(default_vel, env_ids=env_ids)
