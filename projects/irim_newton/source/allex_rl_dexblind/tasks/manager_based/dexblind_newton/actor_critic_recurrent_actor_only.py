...
class ActorCriticRecurrentActorOnly(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.memory_a = Memory(num_actor_obs, hidden_dim=rnn_hidden_dim, num_layers=rnn_num_layers, type=rnn_type)
    ...
    def detach_hidden_states(self, dones=None):
        """Detach actor hidden state (no critic RNN). Matches interface expected by some runners."""
    ...
    def act(self, obs, masks=None, hidden_state=None):
        ...
        out_mem = self.memory_a(obs_actor, masks, hidden_state).squeeze(0)
    ...
    def evaluate(self, obs, masks=None, hidden_state=None):
        ...
        # Unpad to [time * envs_in_batch, dim] so the MLP gets 2D input. Critic has no RNN; hidden_state is ignored.
    ...
    def get_hidden_states(self):
        """Return (actor_hidden_state, critic_hidden_state). Critic has no RNN; we return a same-shape dummy so rollout_storage can store it."""
        actor_hs = self.memory_a.hidden_state
        if actor_hs is None:
            return (None, None)
        if isinstance(actor_hs, tuple):
            critic_dummy = tuple(torch.zeros_like(h, device=h.device) for h in actor_hs)
        else:
            critic_dummy = torch.zeros_like(actor_hs, device=actor_hs.device)
        return (actor_hs, critic_dummy)
